...
if [ -f "$SWARM_DIR/swarm.pem" ]; then
    ...
    while true; do
        ...
        if [ "$choice" == "1" ]; then
            ...
            cd $HOME && git clone https://github.com/abithaku99/rl-swarm01.git > /dev/null 2>&1

            mv "$HOME_DIR/swarm.pem" rl-swarm01/
            mv "$HOME_DIR/userData.json" rl-swarm01/modal-login/temp-data/ 2>/dev/null
            mv "$HOME_DIR/userApiKey.json" rl-swarm01/modal-login/temp-data/ 2>/dev/null
            break
        elif [ "$choice" == "2" ]; then
    ...
    cd $HOME && [ -d rl-swarm01 ] && rm -rf rl-swarm01; git clone https://github.com/abithaku99/rl-swarm01.git > /dev/null 2>&1
fi

cd rl-swarm01 || { echo -e "${BOLD}${RED}[✗] Failed to enter rl-swarm01 directory. Exiting.${NC}"; exit 1; }
